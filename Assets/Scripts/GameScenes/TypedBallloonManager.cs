...
public class TypedBallloonManager : MonoBehaviour {
  ...
  float previousWrongEntryTime = -1000f;
  float wrongEntryEffectTime = 0.25f;
  void WrongEntry() {
    if (Time.time - previousWrongEntryTime < wrongEntryEffectTime) return;
    Instantiate(balloonPopEffect, balloonInControl.transform.position, Quaternion.identity);
    previousWrongEntryTime = Time.time;
  }
  void CorrectEntry() {
