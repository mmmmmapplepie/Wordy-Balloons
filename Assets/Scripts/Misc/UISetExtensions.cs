...
public static class UISetExtensions {
	...
	private static readonly MethodInfo inputFieldSetMethod;
	private static readonly MethodInfo dropdownSetMethod;
	...
	static UISetExtensions() {
		...
		toggleSetMethod = FindSetMethod(typeof(Toggle), "Set");

		// Find the Slider's set method
		sliderSetMethod = FindSetMethod(typeof(Slider), "Set");

		// Find the Scrollbar's set method
		scrollbarSetMethod = FindSetMethod(typeof(Scrollbar), "Set");

		inputFieldSetMethod = FindSetMethod(typeof(TMP_InputField), "SetText");

		// Find the Dropdown's value field and its' Refresh method
		dropdownSetMethod = FindSetMethod(typeof(TMPro.TMP_Dropdown), "SetValue");

		dropdownValueField = (typeof(TMPro.TMP_Dropdown)).GetField("m_Value", BindingFlags.NonPublic | BindingFlags.Instance);
		// dropdownRefreshMethod = (typeof(TMPro.TMP_Dropdown)).GetMethod("Refresh", BindingFlags.NonPublic | BindingFlags.Instance);  // Unity 5.2 <= only
	}
	...
	public static void Set(this TMP_InputField instance, string value, bool sendCallback = false) {
		inputFieldSetMethod.Invoke(instance, new object[] { value, sendCallback });
	}

	// public static void Set(this TMPro.TMP_Dropdown instance, int value) {
	...
	// }
	public static void Set(this TMPro.TMP_Dropdown instance, int value, bool sendCallback = false) {
		dropdownSetMethod.Invoke(instance, new object[] { value, sendCallback });
	}

	private static MethodInfo FindSetMethod(System.Type objectType, string methodName) {
