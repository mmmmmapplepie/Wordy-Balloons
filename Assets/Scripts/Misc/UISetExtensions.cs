...
public static class UISetExtensions {
	...
	static UISetExtensions() {
		...
		dropdownValueField = (typeof(TMP_Dropdown)).GetField("m_Value", BindingFlags.NonPublic | BindingFlags.Instance);
		dropdownRefreshMethod = (typeof(TMP_Dropdown)).GetMethod("Refresh", BindingFlags.NonPublic | BindingFlags.Instance);  // Unity 5.2 <= only
	}
	...
	public static void Set(this TMP_Dropdown instance, int value) {
		dropdownValueField.SetValue(instance, value);
